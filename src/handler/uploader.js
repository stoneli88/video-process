...
exports.onDeleteFile = (req, res) => {
    ...
    rimraf_1.default(dirToDelete, error => {
        if (error) {
...
const moveFile = (destinationDir, sourceFile, destinationFile, success, failure) => {
    mkdirp_1.default(destinationDir, function (error) {
    ...
    fs.readdir(chunksDir, function (err, fileNames) {
        ...
        if (err) {
            ...
            appendToStream(destFileStream, chunksDir, fileNames, 0, () => {
                rimraf_1.default(chunksDir, rimrafError => {
                    if (rimrafError) {
                        logger_1.logger.info("Problem deleting chunks dir! " + rimrafError);
                    }
                });
                success();
            }, failure);
